...
export default async function AdminClaimsPage() {
  if (!hasSupabaseConfig || !(await isAdmin())) return <section className="wrap page-section"><div className="card setup-card"><h1 className="display">Administrator access required</h1><Link className="btn-primary" href="/admin">Return to administration</Link></div></section>;
  const client = getAdminSupabase();
  const { data: claims } = await client.from("provider_claims").select("id, license_number, claimant_name, claimant_email, claimant_role, claimant_phone, message, status, created_at").order("created_at", { ascending: false });
