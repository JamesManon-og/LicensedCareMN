...
export default async function OwnerPage() {
  ...
  if (!user?.email) return <section className="wrap page-section"><div className="card setup-card"><h1 className="display">Sign in to manage a claimed listing</h1><p>Use the same work email that was approved in your provider claim.</p><Link className="btn-primary" href="/login">Sign in</Link></div></section>;
  const client = getAdminSupabase();
  const { data: claims } = await client.from("provider_claims").select("license_number").eq("claimant_email", user.email).eq("status", "approved");
