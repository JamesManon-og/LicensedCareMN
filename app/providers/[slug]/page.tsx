  ...
  return (
    <section className="wrap page-section provider-page">
      ...
      <div className="provider-hero card">
        <div className="provider-avatar">{provider.program_name.slice(0, 2).toUpperCase()}</div>
        <div className="provider-heading"><span className="eyebrow">DHS-sourced launch record</span><h1 className="display">{provider.program_name}</h1><p>{provider.company}</p><div className="bcard-tags"><StatusBadge status={provider.status_class} /><span className="badge badge-unverified">Unclaimed listing</span></div></div>
      </div>
      <div className="provider-layout">
        <div>
          <section className="card detail-card"><h2 className="display">License record</h2><dl className="detail-list"><div><dt>Listed status</dt><dd><StatusBadge status={provider.status_class} /> <span>{provider.license_status}</span></dd></div><div><dt>License number</dt><dd>{provider.license_number}</dd></div><div><dt>Service types</dt><dd className="tag-list">{provider.tags.map((tag) => <span className="svctag" key={tag}>{tag}</span>)}</dd></div></dl><p className="status-note">{STATUS_EXPLANATION[provider.status_class]} This is a launch snapshot; confirm current status directly with Minnesota DHS.</p></section>
          <section className="card detail-card"><h2 className="display">Listed contact details</h2><dl className="detail-list"><div><dt>Address</dt><dd>{provider.address}<br />{provider.city}, MN {provider.zip}<br />{provider.county} County</dd></div><div><dt>Phone</dt><dd>{phoneHref ? <a href={`tel:${phoneHref}`}>{formatPhone(provider.phone)}</a> : "Phone not listed"}</dd></div></dl></section>
      ...
      {related.length > 0 && <section className="related-section"><h2 className="display">Related providers</h2><div className="results-list">{related.map((location) => <ProviderCard key={location.license_number} location={location} />)}</div></section>}
    </section>
  );
}
