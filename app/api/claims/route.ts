...
export async function POST(request: Request) {
  ...
  if (!getAllLocations().some((location) => location.license_number === data.data.licenseNumber)) return NextResponse.json({ error: "That listing was not found in this directory." }, { status: 404 });
  ...
  const { error } = await client.from("provider_claims").insert({
    ...
    claimant_email: data.data.email.toLocaleLowerCase(),
